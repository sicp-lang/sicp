...
There are six ways to create painters:
@itemize{@item{from a constant: @scheme[number->painter]}
              @item{from a color:                  @scheme[color->painter]}
              @item{from a list of line segments:  @scheme[segments->painter]}
              @item{from a list of vectors:        @scheme[vects->painter]}
              @item{from a bitmap:                 @scheme[bitmap->painter]}
              @item{from a procedure:              @scheme[procedure->painter]}}
...
@defproc[(vects->painter [los list-of-vect?]) painter?]{
Constructs a painter that draws a stick figure given by the 
vects (wrt the unit square).}
...
@defproc[(bitmap->painter [b (or/c string? path? (is-a/c? bitmap%))]) painter?]{
Constructs a painter that draws the given bitmap image onto the frame.
}
...
@defproc[(beside3 [p1 painter] [p2 painter] [p3 painter]) painter?]{
...
The procedure paint takes a painter as input
and returns a snip containing the painter's image. A snip is
